...
<%!
   ...
   */
  static final String consumerKey = "selimthegrim";
  static final String consumerSecret = "eba27a8ab783497c";
...
%>
    ...
    <pre><%
	  ...
	  NoteList nl = new NoteList();
          List<?> notelist = nl.getNotes();
	  NoteCollectionCounts ncc = new NoteCollectionCounts();
          for (Object notebook : notebooks) {
            	out.println("Notebook: " + ((Notebook)notebook).getName());
	    	nf.clear();
	    	nf.setNotebookGuid(((Notebook)notebook).getGuid());
		...
		for (Object note : notelist) {
			note = noteStore.getNote(accessToken, ((Note)note).getGuid(), true, false, false, false);
			out.println("Note" + ((Note)note).getGuid() + ": " + ((Note)note).getContent());
		}
          }
...
<% } %>
	...
	<li>
<% if (accessToken != null) { %>
  <a href="?action=listNotebooks">List notes in account</a><br/>
<% } else { %>
